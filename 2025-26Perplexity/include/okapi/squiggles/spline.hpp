...
class SplineGenerator {
  public:
     ...
     *                   path.
     * @param fast If true, the path optimization process will stop as soon as
     * the constraints are met. If false, the optimizer will find the smoothest
     * possible path between the points.
     *
     ...
     * Creates a motion profiled path between the given waypoints.
     *
     * @param iwaypoints The list of vectors that the robot should reach along
     * the path.
     *
  ...
  protected:
    ...
    /**
     * The minimum and maximum durations for a path to take. A larger range
     * allows for longer possible paths at the expense of a longer path
     * generation time.
     */
    const int T_MIN = 2;
    const int T_MAX = 15;
  ...
  public:
     ...
     * SplineGenerator's acceleration and jerk constraints and returns the
     * points that form the curve.
     */
     ...
     *
     * This step creates the velocity and acceleration values to command the
     * robot at each point along the curve.
     */
    ...
    /**
     * Finds the new timestamps for each point along the curve based on the
     * motion profile.
     */
     ...
     *
     * This with interpolate between points on the curve if a point with an
     * exact matching timestamp is not found.
     */
     ...
     */
    void forward_pass(ConstrainedState* predecessor,
    ...
    /**
     * Values that are closer to each other than this value are considered
     * equal.
     */
