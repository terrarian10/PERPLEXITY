...
class MotorGroup : public AbstractMotor {
  public:
     ...
     */
    std::int32_t moveAbsolute(double iposition,
     ...
     * failed, setting errno.
     */
    std::int32_t moveRelative(double iposition,
                              std::int32_t ivelocity) override;
     ...
     *
     * @return The motor's absolute position in its encoder units or
     ...
     * negative direction, and `PROS_ERR` if the operation failed, setting
     * errno.
     */
     ...
     *
     * @return The motor's efficiency in percent or `PROS_ERR_F` if the
     * operation failed, setting errno.
     */
     ...
     * does not provide any value. This function returns `PROS_ERR` with errno
     * set to `ENOSYS`.
     *
    ...
    /**
     * Gets the current limit for the motor in mA. The default value is `2500`
     * mA.
     *
    ...
    /**
     * Sets the reverse flag for the motor. This will invert its movements and
     * the values returned for its position.
     *
     ...
     */
    virtual std::shared_ptr<ContinuousRotarySensor> getEncoder(
        std::size_t index);
