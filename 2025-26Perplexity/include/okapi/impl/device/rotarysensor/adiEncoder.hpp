...
class ADIEncoder : public ContinuousRotarySensor {
  public:
     ...
     * side up. This must be in port ``1``, ``3``, ``5``, or ``7`` (``A``,
     * ``C``, ``E``, or ``G``).
     * @param iportBottom The "bottom" wire from the encoder. This must be in
     * port ``2``, ``4``,
     * ``6``, or ``8`` (``B``, ``D``, ``F``, or ``H``).
     * @param ireversed Whether the encoder is reversed.
     */
    ADIEncoder(std::uint8_t iportTop,
     ...
     * ```
     *
     * @param iports The ports the encoder is plugged in to in the order
     * ``{smart port, top port, bottom port}``. The smart port is the smart port
     * number (``[1, 21]``). The top port is the "top" wire from the encoder
     * with the removable cover side up. This must be in port ``1``,
     * ``3``, ``5``, or
     * ``7`` (``A``, ``C``, ``E``, or ``G``). The bottom port is the "bottom"
     * wire from the encoder. This must be in port ``2``, ``4``, ``6``, or ``8``
     * (``B``, ``D``, ``F``, or ``H``).
     * @param ireversed Whether the encoder is reversed.
     */
