...
class Controller {
  public:
     ...
     * displayed
     * @param icol the column number in the range [0-14] at which the text will
     * be displayed
     * @param itext the string to display
     * @return 1 if the operation was successful, PROS_ERR otherwise
     */
    virtual std::int32_t setText(std::uint8_t iline,
                                 std::uint8_t icol,
                                 std::string itext);
     ...
     * ' ', where dots are short rumbles, dashes are long rumbles, and spaces
