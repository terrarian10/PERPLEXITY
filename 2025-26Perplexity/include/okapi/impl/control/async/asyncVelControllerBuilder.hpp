...
class AsyncVelControllerBuilder {
  public:
     ...
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withMotor(
     ...
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withSensor(
     ...
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withGains(
        const IterativeVelPIDController::Gains& igains);

    /**
     * Sets the VelMath which calculates and filters velocity. This is ignored
     * when using integrated controller. If using a PID controller (by setting
     * the gains), this is required.
     *
     ...
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withDerivativeFilter(
     ...
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withGearset(
        const AbstractMotor::GearsetRatioPair& igearset);

    /**
     * Sets the maximum velocity. The default maximum velocity is derived from
     * the motor's gearset. This parameter is ignored when using an
    ...
    /**
     * Sets the TimeUtilFactory used when building the controller. The default
     * is the static TimeUtilFactory.
     *
     * @param itimeUtilFactory The TimeUtilFactory.
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withTimeUtilFactory(
     ...
     * @return An ongoing builder.
     */
    AsyncVelControllerBuilder& withLogger(
     ...
     * `initialize` and `competition_initialize` tasks are never parented to.
  ...
  private:
    ...
    bool sensorsSetByUser{
        false
    }; // Used so motors don't overwrite sensors set manually
    std::shared_ptr<RotarySensor> sensor;

    bool hasGains{
        false
    }; // Whether gains were passed, no gains means integrated control
    ...
    bool gearsetSetByUser{
        false
    }; // Used so motor's don't overwrite a gearset set manually
