...
class XDriveModel : public ChassisModel {
  public:
     ...
     * motors are powered +100%, the robot should move forward in a straight
     * line.
     *
     ...
     * istrafeSpeed is to the right and negative istrafeSpeed is to the left.
     * Uses velocity mode. The algorithm is (approximately): topLeftPower = -1 *
     * istrafeSpeed + yaw topRightPower = istrafeSpeed - yaw bottomRightPower =
     * -1 * istrafeSpeed - yaw bottomLeftPower = istrafeSpeed + yaw
     *
    ...
    /**
     * Drive the robot with a curvature drive layout. The robot drives in
     * constant radius turns where you control the curvature (inverse of radius)
     * you drive in. This is advantageous because the forward speed will not
     * affect the rate of turning. The algorithm switches to arcade if the
     * forward speed is 0. Uses voltage mode.
     *
     ...
     * @param ithreshold deadband on joystick values
     */
    virtual void xArcade(double irightSpeed,
                         double iforwardSpeed,
                         double iyaw,
                         double ithreshold = 0);

    /**
     * Drive the robot with a field-oriented arcade drive layout. Uses voltage
     * mode. For example: Both `fieldOrientedXArcade(1, 0, 0, 0_deg)` and
     * `fieldOrientedXArcade(1, 0, 0, 90_deg)` will drive the chassis in the
     * forward/north direction. In other words, no matter the robot's heading,
     * the robot will move forward/north when you tell it to move forward/north
     * and will move right/east when you tell it to move right/east.
     *
     *
     ...
     * @param iangle current chassis angle (`0_deg` = no correction, winds
     * clockwise)
     * @param ithreshold deadband on joystick values
     */
