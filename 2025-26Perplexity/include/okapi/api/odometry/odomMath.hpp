...
class OdomMath {
  public:
    ...
    /**
     * Computes the distance and angle from the given Odometry state to the
     * given point. The point and the OdomState must be in
     ...
     */
    static std::pair<QLength, QAngle> computeDistanceAndAngleToPoint(
        const Point& ipoint,
        const OdomState& istate);
