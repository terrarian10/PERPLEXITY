...
class AsyncWrapper : virtual public AsyncController<Input, Output> {
  public:
     ...
     * `AsyncController` by running it in another task. The input controller
     * will act like an `AsyncController`.
     *
     * @param iinput controller input, passed to the `IterativeController`
     * @param ioutput controller output, written to from the
     * `IterativeController`
    ...
    AsyncWrapper<Input, Output>& operator=(
        AsyncWrapper<Input, Output>&& other) = delete;
     ...
     */
    Input getTarget() override { return controller->getTarget(); }
     ...
     */
    Output getOutput() const { return controller->getOutput(); }

    /**
     * Returns the last error of the controller. Does not update when disabled.
     */
    Output getError() const override { return controller->getError(); }
     ...
     */
    Output getMaxOutput() { return controller->getMaxOutput(); }
     ...
     */
    Output getMinOutput() { return controller->getMinOutput(); }
     ...
     * after it was off will cause the controller to move to its last set
     * target, unless it was reset in that time.
     */
     ...
     * it was off will cause the controller to move to its last set target,
     ...
     */
    bool isDisabled() const override { return controller->isDisabled(); }

    /**
     * Blocks the current task until the controller has settled. Determining
     * what settling means is implementation-dependent.
     */
     ...
     * This method is called by the AsyncControllerFactory when making a new
  ...
  protected:
     ...
     * if the controller is turned off, reset, and turned back on.
     */
