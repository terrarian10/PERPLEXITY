...
class AsyncMotionProfileController
  ...
  public:
    ...
    AsyncMotionProfileController& operator=(
        AsyncMotionProfileController&& other) = delete;
     ...
     * pathId to run it.
     *
     * If the waypoints form a path which is impossible to achieve, an instance
     * of `std::runtime_error` is thrown (and an error is logged) which
     * describes the waypoints. If there are no waypoints, no path is generated.
     *
     ...
     * okapi::State::FRAME_TRANSFORMATION format where +x is forward, +y is
     * right, and 0 theta is measured from the +x axis to the +y axis.
     *
     * @param iwaypoints The waypoints to hit on the path.
     * @param ipathId A unique identifier to save the path with.
    ...
    /**
     * Executes a path with the given ID. If there is no path matching the ID,
     * the method will return. Any targets set while a path is being followed
     * will be ignored.
     *
     * @param ipathId A unique identifier for the path, previously passed to
     * `generatePath()`.
    ...
    /**
     * Generates a new path from the position (typically the current position)
     * to the target and blocks until the controller has settled. Does not save
     ...
     * perfectly follow the path. Subclasses can override this to be more
    ...
    /**
     * Returns whether the controller has settled at the target. Determining
     * what settling means is implementation-dependent.
     *
    ...
    /**
     * This implementation does nothing because the maximum velocity is
     * configured using PathfinderLimits elsewhere.
     *
     ...
     * This method is called by the `AsyncMotionProfileControllerBuilder` when
    ...
    /**
     * Saves a generated path to a file. Paths are stored as `<ipathId>.csv`. An
     * SD card must be inserted into the brain and the directory must exist.
     * `idirectory` can be prefixed with
     ...
     * `/usd/` is automatically prepended to `idirectory` if it is not
     * specified.
     *
  ...
  protected:
     ...
     */
    virtual void executeSinglePath(
