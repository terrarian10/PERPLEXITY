...
class FlywheelSimulator {
  public:
     ...
     * changes as the link rotates (by default, you can set a new torque
     * function with setExternalTorqueFunction()).
     */
     ...
     *
     * For example, the default torque function has the torque due to gravity
     * vary as the link swings:
     * [](double angle, double mass, double linkLength) {
     *   return (linkLength * std::cos(angle)) * (mass * -1 * gravity);
     * }
     *
     * @param itorqueFunc the torque function. The return value is the torque
