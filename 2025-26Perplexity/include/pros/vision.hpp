...
class Vision : public Device {
  ...
  public:
     ...
     * \code
     * void opcontrol() {
     *   pros::Vision vision_sensor(1); // Creates a vision sensor on port one,
     * with the zero point set to top left
     * }
     * \endcode
     */
     ...
     *        Signature type
     *
     * \return A vision_signature_s_t that can be set using
     * Vision::set_signature
     *
     * \b Example
     * \code
     ...
     *   vision_signature_from_utility(EXAMPLE_SIG, 8973, 11143, 10058, -2119,
     * -1053, -1586, 5.4, 0); vision_sensor.set_signature(EXAMPLE_SIG,
     * &RED_SIG); while (true) { vision_signature_s_t rtn =
     * vision_sensor.get_by_sig(VISION_PORT, 0, EXAMPLE_SIG);
     ...
     *
     * \return A vision_color_code_t object containing the color code
     * information.
     *
     * \b Example
     * \code
     ...
     * void opcontrol() {
     *   pros::Vision vision_sensor(VISION_PORT);
     *   vision_color_code_t code1 =
     * vision_sensor.create_color_code(EXAMPLE_SIG, OTHER_SIG);
     * }
     * \endcode
     */
    vision_color_code_t create_color_code(
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Vision> vision_all = pros::Vision::get_all_devices();  //
     * All vision sensors that are connected
     * }
     * \endcode
     */
     ...
     *
     * \return The vision_object_s_t object corresponding to the given size id,
     * or PROS_ERR if an error occurred.
     *
     * \b Example
     * \code
     * #define VISION_PORT 1
     ...
     *
     * \return The vision_object_s_t object corresponding to the given color
     * code and size_id, or PROS_ERR if an error occurred.
     *
     * \b Example
     * \code
     * #define VISION_PORT 1
     ...
     * 	 pros::Vision vision_sensor(VISION_PORT);
     *   vision_color_code_t code1 =
     * vision_sensor.create_color_code(EXAMPLE_SIG, OTHER_SIG); while (true) {
     ...
     *
     * \return A vision_signature_s_t containing information about the
     * signature.
     *
     * \b Example
     * \code
     ...
     * sensor. Returns PROS_ERR if the port was invalid, an error occurred, or
     * fewer objects than size_id were found. All objects in object_arr that
     * were not found are given VISION_OBJECT_ERR_SIG as their signature.
     *
     * \b Example
     * \code
     * #define VISION_PORT 1
     ...
     *   vision_object_s_t object_arr[NUM_VISION_OBJECTS];
     *   while (true) {
     *     vision_sensor.read_by_sig(0, EXAMPLE_SIG, NUM_VISION_OBJECTS,
     * object_arr); printf("sig: %d", object_arr[0].signature);
     *     // Prints "sig: 1"
     *     delay(2);
     *   }
     * }
     ...
     *   vision_object_s_t object_arr[NUM_VISION_OBJECTS];
     *   vision_color_code_t code1 =
     * vision_sensor.create_color_code(EXAMPLE_SIG, OTHER_SIG, 0, 0, 0); while
     * (true) { vision_sensor.read_by_code(0, code1, NUM_VISION_OBJECTS,
     * object_arr); printf("sig: %d", object_arr[0].signature);
     *     // Prints the signature of the largest object found
     *     delay(2);
     *   }
     ...
     * function only be used to configure the sensor at the beginning of its
     * use.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     *
     * \note The first time this function is called it returns the vision sensor
     * at the lowest port If this function is called multiple times, it will
     * cycle through all the ports. For example, if you have 1 vision sensor on
     * the robot this function will always return a vision sensor object for
     * that port. If you have 2 vision sensors, all the odd numered calls to
     * this function will return objects for the lower port number, all the even
     * number calls will return vision objects for the higher port number
     *
     *
     * This functions uses the following values of errno when an error state is
     * reached:
     ...
     *
     * \return A vision object corresponding to a port that a vision sensor is
     * connected to the brain If no vision sensor is plugged in, it returns a
     * vision sensor on port PROS_ERR_BYTE
     *
     */
