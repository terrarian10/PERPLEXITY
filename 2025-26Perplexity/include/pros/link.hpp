...
class Link : public Device {
  ...
  public:
    /**
     * Initializes a link on a radio port, with an indicated type. There might
     * be a 1 to 2 second delay from when this function is called to when the
     * link is initializes.
     *
     * \note This function uses the following values of errno when an error
     * state is reached: ENXIO - The given value is not within the range of V5
     * ports (1-21). ENODEV - The port cannot be configured as a radio. ENXIO -
     * The sensor is still calibrating, or no link is connected via the radio.
     *
     * \param port
     ...
     * receiver, with the transmitter having double the transmitting bandwidth
     * as the receiving end (1040 bytes/s vs 520 bytes/s).
     * \param ov
     * 		Indicates if the radio on the given port needs vexlink to
     * override the controller radio. Defualts to True.
     *
     * \return PROS_ERR if initialization fails, 1 if the initialization
     * succeeds.
     *
     * \b Example:
     * \code
     ...
     * \endcode
     */
    explicit Link(const std::uint8_t port,
     ...
     * ports (1-21). ENODEV - The port cannot be configured as a radio. ENXIO -
     * The sensor is still calibrating, or no link is connected via the radio.
     * EBUSY - The transmitter buffer is still busy with a previous
     ...
     * the FIFO buffer or destination buffer.
     *
     * \param dest
