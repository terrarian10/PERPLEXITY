...
class Distance : public Device {
  ...
  public:
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Distance> distance_all = pros::Distance::get_all_devices();
     * // All distance sensors that are connected
     * }
     * \endcode
     */
     ...
     * Distance [port: (port number), distance: (distance), confidence:
     * (confidence), object size: (object size), object velocity: (object
     * velocity)]
     */
...
namespace literals {
/**
 * Constructs a Distance sensor object from a literal ending in _dist via
