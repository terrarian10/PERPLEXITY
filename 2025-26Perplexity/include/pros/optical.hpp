...
class Optical : public Device {
  ...
  public:
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Optical> optical_all = pros::Optical::get_all_devices(); //
     * All optical sensors that are connected
     * }
     * \endcode
     */
     ...
     * void opcontrol() {
     * 		pros::Optical optical(1);
     * 		std::cout << "Saturation: " << optical.get_saturation() <<
     * std::endl;
     * }
     * \endcode
     */
     ...
     * void opcontrol() {
     * 		pros::Optical optical(1);
     * 		std::cout << "Brightness: " << optical.get_brightness() <<
     * std::endl;
     * }
     * \endcode
     */
     ...
     * 			std::cout << "Brightness: " << rgb.brightness <<
     * std::endl; pros::delay(20);
     * 		}
     * }
     * \endcode
     */
     ...
     * ENODEV - The port cannot be configured as an Optical Sensor
     *
     * \return raw rgb value if the operation was successful or an
     * optical_raw_s_t with all fields set to PROS_ERR if the operation failed,
     * setting errno.
     *
     * \b Example:
     * \code{.cpp}
     ...
     * 		pros::Optical optical(1);
     * 		while(1) {
     * 			std::cout << "Gesture: " << optical.get_gesture() <<
     * std::endl; pros::delay(20);
     * 		}
     * }
     * \endcode
     ...
     * 		while(1) {
     * 			pros::c::optical_gesture_s_t gesture =
     *optical.get_gesture_raw(); std::cout << "Gesture raw data: " << std::endl;
     * 			std::cout << "Up data: " << gesture.udata << std::endl;
     * 			std::cout << "Down data: " << gesture.ddata <<
     *std::endl; std::cout << "Left data: " << gesture.ldata << std::endl;
     * 			std::cout << "Right data: " << gesture.rdata <<
     *std::endl; std::cout << "Type: " << gesture.type << std::endl; std::cout
     *<< "Count: " << gesture.count << std::endl; std::cout << "Time: " <<
     *gesture.time << std::endl; pros::delay(20);
     * 		}
     * }
     * \endcode
     */
     ...
     * brightness: (brightness), proximity: (proximity), rgb: {red, green,
     * blue}]
     *
     * \b Example:
     * \code{.cpp}
