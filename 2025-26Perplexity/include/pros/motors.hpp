...
class Motor
  ...
  public:
     ...
     *
     * \param port
     *        The V5 port number from 1 to 21, or from -21 to -1 for reversed
     * motors. A reversed motor will reverse the input or output movement
     * functions and movement related telemetry in order to produce consistant
     * behavior with non-reversed motors
     *
     * \param gearset = pros::v5::MotorGears::green
     * 		  Optional parameter for the gearset for the motor.
     * 		  Does not explicitly set the gearset if not specified or if the
     * gearset is invalid
     *
     ...
     * \code
     * void opcontrol() {
     * 	Motor first_motor(1); //Creates a motor on port 1 without altering
     * gearset or encoder units Motor reversed_motor(-2); //Creates a reversed
     * motor on port 1 port 1 without altering gearset or encoder units Motor
     * blue_motor(3, pros::v5::MotorGears::blue); //Creates a motor on port 3
     * with blue gear set Motor rotations_motor(4, pros::v5::MotorGears::green,
     * pros::v5::MotorUnits::rotations); //port 4 w/ rotations
     *
     * }
     * \endcode
     *
     ...
     * parameter would result in the motor moving clockwise 10 units (counter
     * clockwise if reversed), no matter what the current position is.
     *
     * \note This function simply sets the target for the motor, it does not
     * block program execution until the movement finishes.
     *
     ...
     *
     * \note This is one of many Motor functions that takes in an optional index
     * parameter. This parameter can be ignored by most users but exists to give
     * a shared base class for motors and motor groups
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENODEV - The port cannot be configured as a motor
     ...
     * 		  The zero-indexed index of the motor to get the target position
     * of. By default index is 0, and will return an error for a non-zero index
     *
     ...
     *   pros::Motor motor (1);
     *   while (true) {
     *     motor = controller_get_analog(E_CONTROLLER_MASTER,
     * E_CONTROLLER_ANALOG_LEFT_Y); printf("Actual velocity: %lf\n",
     ...
     * \note This is one of many Motor functions that takes in an optional index
     parameter.
     * 		 This parameter can be ignored by most users but exists to give
     a shared base class
     ...
     * 		  The zero-indexed index of the motor to get the target position
     of.
     * 		  By default index is 0, and will return an error for a non-zero
     index
     *
     ...
     * motor.is_over_current(); pros::delay(2);
     *   }
     * }
     * \endcode
     */
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Motor> motor_all = pros::Motor::get_all_devices();  // All
    ...
    /// These functions allow for motors and motor groups to be used
    /// interchangeably
    ///@{
    /**
     * Gets a vector containing the target position set for the motor by the
     * user
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     *
     *
     * \return A vector containing the target position in its encoder units or
     ...
     *   while (true) {
     *     motor.move_velocity(master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y));
     *     std::cout << "Motor Velocity: " <<
     * motor.get_target_velocity_all()[0];
     *     // Prints the value of E_CONTROLLER_ANALOG_LEFT_Y
     *     pros::delay(2);
     *   }
     * }
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing the motor's actual velocity in RPM or
     * PROS_ERR_F if the operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     *     motor.move_velocity(master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y));
     *     std::cout << "Motor Velocity: " <<
     * motor.get_actual_velocity_all()[0];
     *     // Prints the value of E_CONTROLLER_ANALOG_LEFT_Y
     *     pros::delay(2);
     *   }
     * }
     ...
     *
     *
     * \return A vector containing the motor's current in mA or PROS_ERR if the
     * operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     *   while (true) {
     *     motor = master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y);
     *     std::cout << "Motor Current Draw: " <<
     * motor.get_current_draw_all()[0]; pros::delay(2);
     *   }
     * }
     * \endcode
     */
     ...
     *
     *
     * \return A vector containing The motor's efficiency in percent or
     * PROS_ERR_F if the operation failed, setting errno.
     *
     * \b Example
     * \code
    ...
    /**
     * Gets a vector containing the absolute position of the motor in its
     encoder units.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     *
     * \return A vector containing the motor's absolute position in its encoder
     units or PROS_ERR_F
     * if the operation failed, setting errno.
     *
     * \b Example
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing the motor's power draw in Watts or PROS_ERR_F
     * if the operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     * was read will not be supplied
     *
     * \return A vector containing the raw encoder count at the given timestamp
     * or PROS_ERR if the operation failed.
     *
     * \b Example
     * \code
     * void opcontrol() {
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing the motor's torque in Nm or PROS_ERR_F if the
     * operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     *
     *
     * \return A vector of the motor's voltage in mV or PROS_ERR_F if the
     * operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing 1 if the motor's current limit is being
     * exceeded and 0 if the current limit is not exceeded, or PROS_ERR if the
     * operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing 1 if the temperature limit is exceeded and 0
     * if the temperature is below the limit, or PROS_ERR if the operation
     * failed, setting errno.
     *
     * \b Example
     * \code
     ...
     *     motor = master.get_analog(E_CONTROLLER_ANALOG_LEFT_Y);
     *     std::cout << "Is the motor over its temperature limit?: " <<
     * motor.is_over_temp_all(); pros::delay(2);
     *   }
     * }
     * \endcode
     */
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing the motor's current limit in mA or PROS_ERR
     * if the operation failed, setting errno.
     *
     * \b Example
     * \code
     ...
     *   pros::Motor motor (1);
     *   while (true) {
     *     std::cout << "Motor Current Limit: " <<
     * motor.get_current_limit_all()[0]; pros::delay(2);
     *   }
     * }
     * \endcode
     */
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing One of Motor_Units according to what is set
     * for the motor or E_MOTOR_ENCODER_INVALID if the operation failed.
     *
     * \b Example
     * \code
     * void initialize() {
     *   pros::Motor motor (1, E_MOTOR_GEARSET_06, E_MOTOR_ENCODER_COUNTS);
     *   std::cout << "Motor Encoder Units: " <<
     * motor.get_encoder_units_all()[0];
     * }
     * \endcode
     */
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing one of Motor_Gears according to what is set
     * for the motor, or pros::Motor_Gears::invalid if the operation failed.
     *
     * \b Example
     * \code
     * void initialize() {
     ...
     *
     * \return A vector containing the signed port of the motor. (negative if
     ...
     * ENODEV - The port cannot be configured as a motor
     *
     * \return A vector containing the motor's voltage limit in V or PROS_ERR if
     ...
     * void initialize() {
     *   pros::Motor motor (1);
     *   std::cout << "Motor Voltage Limit: " <<
     * motor.get_voltage_limit_all()[0];
     * }
     * \endcode
     */
     ...
     *
     * \return A vector containing 1 if the motor has been reversed and 0 if the
     * motor was not reversed, or PROS_ERR if the operation failed, setting
     * errno.
     *
     * \b Example
     * \code
     ...
     * \endcode
     */
    std::int32_t set_brake_mode_all(
