...
class Port {
  ...
  public:
     ...
     *its valid range (ADI port: 1-8, 'a'-'h', or 'A'-'H'; smart port: 1-21).
     *
     * \param adi_port
     ...
     * \endcode
     */
    explicit Port(std::uint8_t adi_port,
     ...
     *number (from 1-8, 'a'-'h', 'A'-'H') to configure
     * \param type
     * 		  The configuration type for the port
     *
     * \b Example
     ...
     * \endcode
     */
    explicit Port(ext_adi_port_pair_t port_pair,
                  adi_port_config_e_t type = E_ADI_TYPE_UNDEFINED);
     ...
     *
     * \note The parts of the tuple are {smart port, adi port, second adi port
     * (when applicable)}.
     *
     *
     * \b Example
     ...
     *
     * 	 // Prints the first and second value from the port tuple (The Adi Port.
     * The first value is the Smart Port) printf("Sensor Smart Port: %d\n",
...
class AnalogIn : protected Port {
  ...
  public:
     ...
     * apart, for a 0.5 s period of calibration. The average value thus
     ...
     * will return the difference between this value and the current sensor
     * value when called.
     *
     * Do not use this function when the sensor value might be unstable (gyro
     * rotation, accelerometer movement).
     *
     * \note The ADI currently returns data at 10ms intervals, in contrast to
     * the calibrate function’s 1ms sample rate.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * reduce drift due to round-off, and should not be used on a sensor such as
     * a line tracker or potentiometer.
     *
     * The value returned actually has 16 bits of "precision", even though the
     * ADC only reads 12 bits, so that error induced by the average value being
     * between two values when integrated over time is trivial. Think of the
     * value as the true value times 16.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENODEV - The port is not configured as an analog input
     ...
     * different mode. The meaning of the returned value varies depending on the
     * sensor attached.
     *
     * Inherited from ADIPort::get_value.
     *
     * This function uses the following values of errno when an error state is
     * reached: EADDRINUSE - The port is not configured as an analog input (e.g.
     ...
     * AnalogIn [smart_port: analog_in._smart_port, adi_port:
     * analog_in._adi_port, value calibrated: (12 bit calibrated value), value
     * calibrated HR: (16 bit calibrated value), value: (12 bit value)]
     */
    friend std::ostream& operator<<(std::ostream& os,
                                    pros::adi::AnalogIn& analog_in);
...
class AnalogOut : private Port {
  ...
  public:
     ...
     *
     * This function uses the following values of errno when an error state is
     * reached: EACCES - Another resource is currently trying to access the ADI.
     *
     * \param value
     *        The value to set the ADI port to
     *
     ...
     *
     * Prints in format(this below is all in one line with no new line):
     * AnalogOut [smart_port: analog_out._smart_port, adi_port:
     * analog_out._adi_port, value: (value)]
     */
    friend std::ostream& operator<<(std::ostream& os,
                                    pros::adi::AnalogOut& analog_out);
};
///@}
...
class DigitalOut : private Port {
  ...
  public:
     ...
     * This function uses the following values of errno when an error state is
     * reached:
     * EADDRINUSE - The port is not configured as a digital output (e.g. the
     * port has been reconfigured)
     *
     * \param value
     *        The value to set the ADI port to
     *
     ...
     *
     * Prints in format(this below is all in one line with no new line):
     * DigitalOut [smart_port: digital_out._smart_port, adi_port:
     * digital_out._adi_port, value: (value)]
     */
    friend std::ostream& operator<<(std::ostream& os,
...
class DigitalIn : private Port {
  ...
  public:
     ...
     * the same circumstances, so only one task should call this function for
     * any given button. E.g., Task A calls this function for buttons 1 and 2.
     * Task B may call this function for button 3, but should not for buttons 1
     * or 2. A typical use-case for this function is to call inside opcontrol to
     * detect new button presses, and not in any other tasks.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * reached:
     *
     * EADDRINUSE - The port is not configured as a digital input (e.g. the port
     ...
     *
     * Prints in format(this below is all in one line with no new line):
     * DigitalIn [smart_port: digital_in._smart_port, adi_port:
     * digital_in._adi_port, value: (value)]
     */
    friend std::ostream& operator<<(std::ostream& os,
                                    pros::adi::DigitalIn& digital_in);
...
class Motor : private Port {
  ...
  public:
     ...
     *   std::cout << "Commanded Motor Power: " << motor.get_value(); // Will
     *display 127 delay(1000); motor.set_value(0); // Stop the motor
     * }
     * \endcode
     */
     ...
     *   motor.set_value(127); // Go full speed forward
     *   std::cout << "Commanded Motor Power: " << motor.get_value(); // Will
     * display 127 delay(1000); motor.stop(); // Stop the motor
     * }
     * \endcode
     */
...
class Encoder : private Port {
  ...
  public:
     ...
     * \endcode
     */
    explicit Encoder(std::uint8_t adi_port_top,
     ...
     *
     * \param port_tuple
     *        The tuple of the smart port number, the "top" wire from the
     *encoder sensor with the removable cover side up, and the "bottom" wire
     ...
     *
     * void opcontrol() {
     *   pros::adi::Encoder sensor ({ SMART_PORT, PORT_TOP, PORT_BOTTOM },
     *false);
     *   // Use the sensor
     * }
     * \endcode
...
class Ultrasonic : private Port {
  ...
  public:
     ...
     *port 1, 3, 5, or 7 ('A', 'C', 'E', 'G').
     * \param port_echo
     *        The port connected to the yellow INPUT cable. This should be in
     *the next highest port following port_ping.
     *
     * \b Example
     * \code
     * #define PORT_PING 1
     ...
     *
     * \param port_tuple
     *        The tuple of the smart port number, the port connected to the
     *orange OUTPUT cable (1, 3, 5, 7 or 'A', 'C', 'E', 'G'), and the port
     ...
     * never started, the return value is undefined. Round and fluffy objects
...
class Gyro : private Port {
  ...
  public:
    ...
    /**
     * Initializes a gyroscope on the given port of an adi expander. If the
     *given port has not previously been configured as a gyro, then this
     *function starts a 1300ms calibration period.
     *
     * It is highly recommended that an adi::Gyro object be created in
     *initialize() when the robot is stationary to ensure proper calibration. If
     *an adi::Gyro object is declared at the global scope, a hardcoded 1300ms
     *delay at the beginning of initialize will be necessary to ensure that the
     *gyro's returned values are correct at the beginning of
...
class Potentiometer : public AnalogIn {
  ...
  public:
     ...
     *
     * void opcontrol() {
     *   pros::adi::Potentiometer potentiometer (POTENTIOMETER_PORT,
     *POTENTIOMETER_TYPE); while (true) {
     *     // Get the potentiometer angle
     ...
     *
     * void opcontrol() {
     *   pros::adi::Potentiometer potentiometer ({{ SMART_PORT ,
     *ADI_POTENTIOMETER_PORT }}); while (true) {
     *     // Get the potentiometer angle
     *     std::cout << "Angle: " << potentiometer.get_angle();
     *     pros::delay(10);
     *   }
     ...
     * between 0-250 degrees. Potentiometer V2 rotates 330 degrees thus
     * returning an angle between 0-330 degrees.
     *
     * This function uses the following values of errno when an error state is
     * reached:
...
class Led : protected Port {
  ...
  public:
     ...
     * recommended that you call .update(); after doing any operations with
     * this.
     *
    ...
    /**
     * @brief Force the LED strip to update with the current buffered values,
     * this should be called after any changes to the buffer using the []
     * operator.
     *
     * This function uses the following values of errno when an error state is
     * reached:
...
class Pneumatics : public DigitalOut {
  ...
  public:
     ...
     * 		  If true, the pneumatic will start extended when the program
     *starts. By default, the piston starts retracted when the program starts.
     * \param extended_is_low
     * 		  A flag to set whether the the pneumatic is extended when the
     *ADI it receives a high or a low value. When true, the extended state
     * 		  corresponds to a output low on the ADI port. This allows the
     *user to reverse the behavior of the pneumatics if needed.
     *
     * /b Example:
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics left_piston('a', false);			//
     *Starts retracted, extends when the ADI port is high pros::adi::Pneumatics
     *right_piston('b', false, true);	// Starts retracted, extends when the
     *ADI port is low
     *
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics left_piston({1, 'a'}, false);
     *	// Starts retracted, extends when the ADI port is high
     *   pros::adi::Pneumatics right_piston({1, 'b'}, false, true);	    //
     ...
     * \code
     * void opcontrol() {
     * 	 pros::adi::Pneumatics piston({1, 'a'}, false);            // Starts
     * retracted, extends when the ADI port is high
     *
     *   pros::Controller master(pros::E_CONTROLLER_MASTER);
     *
     *   while (true) {
