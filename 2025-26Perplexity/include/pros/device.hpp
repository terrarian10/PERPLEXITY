...
enum class DeviceType {
    ...
    undefined =
        255 ///< The device type is not defined, or is not a valid device
};
...
class Device {
  public:
     ...
     *
     * \param device_type The pros::DeviceType enum that matches the type of
     * device desired.
     *
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Device> motor_devices =
     * pros::Device::get_all_devices(pros::DeviceType::motor);  // All Device
     * objects are motors
     * }
     * \endcode
     */
    ...
    static std::vector<Device> get_all_devices(
        pros::DeviceType device_type = pros::DeviceType::undefined);
