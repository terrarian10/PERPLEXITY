...
class Gps : public Device {
  ...
  public:
     ...
     *        The V5 port number from 1-21
     * \param xInitial
     * 				 Cartesian 4-Quadrant X initial position
     * (meters)
     * \param yInitial
     * 				 Cartesian 4-Quadrant Y initial position
     * (meters)
     ...
     * at the center of the field (meters)
     * \param headingInitial
     * 				 Initial Heading, with 0 being North, 90 being
     * East, 180 being South, and 270 being West (degrees)
     * \param xOffset
     * 				 Cartesian 4-Quadrant X offset from center of
     * turning (meters)
     * \param yOffset
     ...
     * at the center of the field (meters)
     * \param  headingInitial
     *  			 Heading with 0 being north on the field, in
     * degrees [0,360) going clockwise
     ...
     * \endcode
     */
    virtual std::int32_t initialize_full(double xInitial,
                                         double yInitial,
                                         double headingInitial,
                                         double xOffset,
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Gps> gps_all = pros::Gps::get_all_devices();  // All GPS
    ...
    /**
     * Get the GPS's cartesian location relative to the center of turning/origin
     * in meters.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * \endcode
     */
    virtual std::int32_t set_position(double xInitial,
                                      double yInitial,
                                      double headingInitial) const;

    /**
     * Set the GPS sensor's data rate in milliseconds, only applies to IMU on
     *GPS.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Get the possible RMS (Root Mean Squared) error in meters for GPS
     *position.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * EAGAIN - The sensor is still calibrating
     *
     * \return Possible RMS (Root Mean Squared) error in meters for GPS
     *position. If the operation failed, returns PROS_ERR_F and errno is set.
     *
     * \b Example
     * \code
     * #define GPS_PORT 1
    ...
    /**
     * Gets the X position in meters of the robot relative to the starting
     *position.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets the Y position in meters of the robot relative to the starting
     *position.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets the pitch of the robot in degrees relative to the starting
     *oreintation.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets the roll of the robot in degrees relative to the starting
     *oreintation.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets the yaw of the robot in degrees relative to the starting
     *oreintation.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * EAGAIN - The sensor is still calibrating
     *
     * \return The raw gyroscope value in z-axis. If the operation fails,
     returns
     * PROS_ERR_F and errno is set.
     *
     \b Example
     ...
     * EAGAIN - The sensor is still calibrating
     *
     * \return The raw gyroscope value in x-axis. If the operation fails,
     *returns PROS_ERR_F and errno is set.
     *
     * \b Example
     * \code
     ...
     * EAGAIN - The sensor is still calibrating
     *
     * \return The raw gyroscope value in y-axis. If the operation fails,
     *returns PROS_ERR_F and errno is set.
     *
     * \b Example
     * \code
    ...
    */
     ...
     *
     * Prints in format:
     * Gps [port: gps._port, x: (x position), y: (y position), heading: (gps
     *heading), rotation: (gps rotation)]
     *
     * \b Example
     * \code
     * #define GPS_PORT 1
     ...
     *
     * \note The first time this function is called it returns the gps sensor at
     * the lowest port If this function is called multiple times, it will cycle
     * through all the ports. For example, if you have 1 gps sensor on the robot
     * this function will always return a gps sensor object for that port.
     * If you have 2 gps sensors, all the odd numered calls to this function
     * will return objects for the lower port number, all the even number calls
     * will return gps objects for the higher port number
     *
     *
     ...
     *
     * \return A gps object corresponding to a port that a gps sensor is
     * connected to the brain If no gps sensor is plugged in, it returns a gps
     * sensor on port PROS_ERR_BYTE
     *
     */
    static Gps get_gps();
    ///@}
 ...
 *     screen_print(TEXT_MEDIUM, 1, "X Position: %4d, Y Position: %4d", pos.x,
 * pos.y); delay(20);
 *   }
 * }
 * \endcode
