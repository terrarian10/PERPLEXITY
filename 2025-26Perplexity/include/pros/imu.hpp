...
class Imu : public Device {
  ...
  public:
     ...
     *
     * \note The first time this function is called it returns the IMU sensor at
     * the lowest port If this function is called multiple times, it will cycle
     * through all the ports. For example, if you have 1 IMU sensor on the robot
     * this function will always return a IMU sensor object for that port.
     * If you have 2 IMU sensors, all the odd numered calls to this function
     * will return objects for the lower port number, all the even number calls
     * will return IMU objects for the higher port number
     *
     *
     ...
     *
     * \return A IMU object corresponding to a port that a IMU sensor is
     * connected to the brain If no IMU sensor is plugged in, it returns a IMU
     ...
     *if the blocking param is true, with a timeout for this operation being set
     *a 3 seconds as a safety margin. This function also blocks until the IMU
     ...
     * EAGAIN - The sensor is already calibrating, or time out setting the
     *status flag.
     *
     * \param blocking
     ...
     *
     * The rate may be specified in increments of 5ms, and will be rounded down
     * to the nearest increment. The minimum allowable refresh rate is 5ms. The
     * default rate is 10ms.
     *
     * As values are copied into the shared memory buffer only at 10ms
     * intervals, setting this value to less than 10ms does not mean that you
     * can poll the sensor's values any faster. However, it will guarantee that
     * the data is as recent as possible.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENXIO - The given value is not within the range of V5 ports (1-21).
     ...
     * \code
     * void opcontrol() {
     *   std::vector<Imu> imu_all = pros::Imu::get_all_devices();  // All IMU
     ...
     * represented with positive degree values, while counterclockwise rotations
     * are represented with negative ones.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENXIO - The given value is not within the range of V5 ports (1-21).
     ...
     *     cout << "Quaternion: " << quat.w << ", " << quat.x << ", " << quat.y
     * << ", " << quat.z << endl; delay(20);
     *   }
     * }
     * \endcode
     */
     ...
     *     cout << "Euler: " << euler.roll << ", " << euler.pitch << ", " <<
     * euler.yaw << endl; delay(20);
     *   }
     * }
     * \endcode
     */
     ...
     *     cout << "Gyro: " << gyro.x << ", " << gyro.y << ", " << gyro.z <<
     * endl; delay(20);
     *   }
     * }
     * \endcode
     ...
     * \param  port
     * 				 The V5 Inertial Sensor port number from 1-21
     * \returns The physical orientation of the Inertial Sensor or PROS_ERR if
     * an error occured.
     *
     */
     ...
     * gyro rate: {x,y,z}, get accel: {x,y,z}, calibrating: (calibrating
     * boolean)]
     */
