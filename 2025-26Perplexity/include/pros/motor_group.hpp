...
class MotorGroup : public virtual AbstractMotor {
  ...
  public:
     ...
     * void opcontrol() {
     * 	MotorGroup first_mg({1, -2}); //Creates a motor on port 1  and a
     * reversed motor on port 2 with with both motors using the green gearset
     * and degrees as the encoder units MotorGroup rotations_mg({4, 5},
     * pros::v5::MotorGears::blue, pros::v5::MotorUnits::rotations);
     *  //Creates a motor group on ports 4 and 5 with blue motors using rotaions
     * as the encoder units
     * }
     ...
     * 	MotorGroup first_mg({1, -2}); //Creates a motor on port 1  and a
     * reversed motor on port 2 with with both motors using the green gearset
     * and degrees as the encoder units AbstractMotor abs_mtr_group = first_mg;
     ...
     *     // Continue running this loop as long as the mg is not within +-5
     ...
     *
     * This movement is relative to the current position of each motor as given
     * in pros::MotorGroup::get_position(). Providing 10.0 as the position
     * parameter would result in the motor moving 10 units, no matter what the
     * current position is.
     *
     * \note This function simply sets the target for the motor, it does not
     * block program execution until the movement finishes.
    ...
    /**
     * Gets a vector of the absolute position of each motor in its encoder
     * units.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets a vector of the power drawn by each motor in the motor group in
     * Watts.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets the temperature of each motor  in the motor group in degrees
     * Celsius.
     *
     * This function uses the following values of errno when an error state is
     * reached:
    ...
    /**
     * Gets a vector of the torque generated by each motor in Newton Meters
     * (Nm).
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * EDOM - The motor group is empty
     *
     * \return A vector containing each motor's torque in Nm or PROS_ERR_F if
    ...
    /**
     * Checks if each motor in the motor group is drawing over its current
     * limit.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * EDOM - The motor group is empty
     *
     * \return A vector containing the following for each motor: 1 if the
     * motor's current limit is being exceeded and 0 if the current limit is not
    ...
    /**
     * Gets a vector with the brake mode that was set for each motor in the
     * motor group.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     ...
     * EDOM - The motor group is empty
     *
     * \return A vector with one of MotorBrake for each motor in the motor
     * group, according to what was set for the motor, or E_MOTOR_BRAKE_INVALID
     * if the operation failed, setting errno.
     *
     * \b Example
     * \code
    ...
    /**
     * Gets a vector of the current limit for each motor in the motor group in
     * mA.
     *
     * The default value is 2500 mA.
     *
     ...
     * mg.get_current_limit_all()[0]; pros::delay(2);
     *   }
     * }
     * \endcode
     */
     ...
     * motor, or pros::MotorGears::invalid if the operation failed for each
     * motor.
     *
     * \b Example
     * \code
    ...
    /**
     * Sets one of MotorBrake to a motor in the motor group. Works with the C
    ...
    std::int32_t set_brake_mode(const pros::motor_brake_mode_e_t mode,
                                const std::uint8_t index = 0) const;
    /**
     * Sets one of MotorBrake all the motors in the motor group. Works with the
     * C enum and the C++ enum class.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENODEV - The port cannot be configured as a motor
    ...
    std::int32_t set_encoder_units(const pros::motor_encoder_units_e_t units,
                                   const std::uint8_t index = 0) const;
    /**
     * Sets one of MotorUnits for every motor in the motor group's motor
     * encoder. Works with the C enum and the C++ enum class.
     *
     * This function uses the following values of errno when an error state is
     * reached:
     * ENODEV - The port cannot be configured as a motor
     ...
     * ENODEV - The port cannot be configured as a motor
     * EDOM - The motor group is empty
     * E2BIG - The size of the vector mismatches the number of motors in the
     * motor group
     *
     ...
     * \endcode
     */
    std::int32_t set_gearing(
        std::vector<pros::motor_gearset_e_t> gearsets) const;
    /**
     * Sets one of the gear cartridge (red, green, blue) for one motor in the
     * motor group. Usable with the C++ enum class and the C enum.
     *
    ...
    std::int32_t set_zero_position(const double position,
                                   const std::uint8_t index = 0) const;
    /**
     * Sets the position for every motor in the motor group in its encoder
     * units.
     *
     * This will be the future reference point for the motor's "absolute"
     * position.
    ...
    /**
     * Sets the "absolute" zero position of every motor in the motor group to
    ...
    /**
     * Removes the all motors on the port (regardless of reversal) from the
     * motor group
     *
