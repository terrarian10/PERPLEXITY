...
#pragma GCC diagnostic pop
    ...
    std::int32_t print(std::uint8_t line,
                       ...
                       const char* fmt,
                       Params... args) {
        return pros::c::controller_print(
            _id, line, col, fmt, convert_args(args)...);
    }
     ...
     *and ' ', where dots are short rumbles, dashes are long rumbles, and spaces
     *are pauses. Maximum supported length is 8 characters.
     *
     * \return 1 if the operation was successful or PROS_ERR if the operation
     * failed, setting errno.
     *
 ...
 *seperated by newlines pros::delay(100); pros::list_files("/test", test, 128);
 *	pros::delay(200);
 *	printf("%s\n", test); //Prints the names of files in the folder named
 *test seperated by newlines pros::delay(100);
 * }
 * \endcode
 */
