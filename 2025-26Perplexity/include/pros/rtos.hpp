...
class Task {
  ...
  public:
     ...
     * facilitate debugging. The name may be up to 32 characters long.
     *
     * \b Example
     * \code
     ...
     *
     * void initialize() {
     *   pros::c::task_t my_task = pros::Task::create(my_task_fn,
     * (void*)"PROS");
     * }
     * \endcode
     */
    template<class F>
    static task_t create(F&& function,
                         std::uint32_t prio = TASK_PRIORITY_DEFAULT,
                         std::uint16_t stack_depth = TASK_STACK_DEPTH_DEFAULT,
                         const char* name = "") {
    ...
    static task_t create(F&& function, const char* name) {
        return Task::create(std::forward<F>(function),
     ...
     * void my_task_fn(void* param) {
     *   while(true) {
     * 	   counter_mutex.take(); // Mutexes are used for protecting shared
     * resources counter++; counter_mutex.give(); pros::delay(10);
     *   }
     * }
     *
     * void opcontrol() {
     ...
     * counter.
     *
     * See https://pros.cs.purdue.edu/v5/tutorials/topical/notifications.html
     * for details.
     *
     ...
     * \param action
     *        An action to optionally perform on the receiving task's
     * notification value
     * \param prev_value
     ...
     * without needing to overwrite, 1 otherwise. For all other NOTIFY_ACTION
     ...
     * requested. To delay cyclically, use task_delay_until().
     *
     * \param milliseconds
    ...
    /**
     * Delays the current Task until a specified time.  This function can be
     ...
     *
     * \return The number of tasks that are currently being managed by the
     * kernel.
     *
     * \b Example
     * \code
...
class Mutex {
  ...
  public:
     ...
     * for details.
     *
     * \return True if the mutex was successfully taken, false otherwise. If
     * false is returned, then errno is set with a hint about why the the mutex
     * couldn't be taken
     *
     * \b Example
     * \code
     * // Global variables for the robot's odometry, which the rest of the
     * robot's
     ...
     *
     * // This mutex protects the odometry data. Whenever we read or write to
     * the
     * // odometry data, we should make copies into the local variables, and
     * read
     ...
     *   while(true) {
     *     // First we fetch the odom coordinates from the previous iteration of
     * the
     *     // odometry task. These are put into local variables so that we can
     *     // keep the size of the critical section as small as possible. This
     * lets
     *     // other tasks that need to use the odometry data run until we need
     * to
     *     // update it again.
     *     odom_mutex.take();
     *     double x_old = odom_x;
     ...
     *     // we can use them without worrying about the odometry task changing
     * say,
     *     // the y value right after we've read the x. This ensures our values
     * are
     *     // sound.
     *     odom_mutex.take();
    ...
    /**
     * Takes and locks a mutex, waiting for up to a certain number of
     ...
     *
     * \param timeout
     *        Time to wait before the mutex becomes available. A timeout of 0
     * can be used to poll the mutex. TIMEOUT_MAX can be used to block
     *        indefinitely.
     *
     * \return True if the mutex was successfully taken, false otherwise. If
     * false is returned, then errno is set with a hint about why the the mutex
    ...
    bool try_lock_for(const std::chrono::duration<Rep, Period>& rel_time) {
        return take(
            std::chrono::duration_cast<Clock::duration>(rel_time).count());
    }
    ...
    bool try_lock_until(
        ...
        return take(std::max(static_cast<uint32_t>(0),
...
class MutexVar {
  ...
  public:
     ...
     *
     * \param timeout
     *        Time to wait before the mutex becomes available, in milliseconds.
     * A timeout of 0 can be used to poll the mutex.
     *
     ...
     *   while(true) {
     *     std::optional<pros::MutexVar<Pose>> cur_pose_opt =
     * odom_pose.try_lock(100);
     *
     ...
     */
    template<typename Rep, typename Period>
    std::optional<MutexVarLock<Var>> try_lock(
